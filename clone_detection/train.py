...
from nets.VisDectNet import Siamese_VGG, Siamese_VGG_attn, Siamese_Resnet
    ...
    try:
        for current_epoch in range(init_epoch, num_epoch):
            ...
            for iteration, batch in enumerate(train_iter):
                ...
                image1 = images[0].unsqueeze(0)
                image2 = images[1].unsqueeze(0)

                images = torch.cat([image1, image2], dim=0).permute(0, 1, 2, 3, 4).float()
                ...
                with torch.no_grad():
                    equal = torch.eq(torch.round(nn.Sigmoid()(outputs)), labels)
            ...
            for iteration, batch in enumerate(valid_iter):
                ...
                with torch.no_grad():
                    ...
                    p, r, f, _ = precision_recall_fscore_support(label, prep, average='binary')
...
def train(config):
    ...
    print("Init model...")
    if network == "vgg":
        model = Siamese_VGG(config['input_shape'])
        num_parameters = count_parameters(model.vgg)
    elif network == "vgg-attn":
        model = Siamese_VGG_attn(config['input_shape'])
        num_parameters = count_parameters(model.vgg)
    else:
        model = Siamese_Resnet(config['input_shape'], dropout)
        num_parameters = count_parameters(model.resnet)
    ...
    train_iter, valid_iter, test_iter = get_dataloader('CD', IR_path,
